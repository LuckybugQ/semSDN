...
public class Node {
	           ...
	           )
	{
		...
		this.logger = java.util.logging.Logger.getLogger("dk.i1.diameter.node");
		this.log = LoggerFactory.getLogger(Node.class);  
	...
	public void start() throws java.io.IOException, UnsupportedTransportProtocolException {
		...
		//log.info("start()方法,开始启动Node节点！");
		// logger.log(Level.INFO,"Starting Diameter node");
		please_stop = false;
//		log.info("读取节点配置中配置的通信协议");
		prepare();
//		log.info("启动通信协议节点NodeImplementation");
		if(tcp_node!=null)
			tcp_node.start();
		if(sctp_node!=null)
			sctp_node.start();
...
//		log.info("TCP节点NodeImplementation加载完成");
